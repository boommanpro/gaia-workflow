...
public class WorkflowNode extends BaseNode {
     ...
     * 这个方法应该在工作流引擎初始化时调用
     */
    public static void setWorkflowStorageService(WorkflowStorageService service) {
        workflowStorageService = service;
    }

    /**
     * 获取目标工作流定义
     */
    private String getTargetWorkflowSchema(String workflowId) {
        if (workflowStorageService == null) {
            log.warn("工作流存储服务未初始化，无法获取工作流定义: {}", workflowId);
            return "{}";
        }

        try {
            WorkflowDefinition workflow = workflowStorageService.getWorkflowById(workflowId);
            if (workflow == null) {
                throw new IllegalArgumentException("工作流不存在: " + workflowId);
            }
            return workflow.getDefinition();
        } catch (Exception e) {
            log.error("获取工作流定义失败: {}", workflowId, e);
            throw new RuntimeException("获取工作流定义失败: " + workflowId, e);
        }
    }
